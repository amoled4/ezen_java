...
public class FileObjectEx01 {
	...
	public static void main(String[] args) {
		...
		System.out.println(f.getPath());          // 파일 경로+이름
		...
		// fstr에서 드라이브만 추출해서 출력
		System.out.println("드라이브 : "+fstr.substring(0, fstr.indexOf("\\")));
		// fstr에서 파일명만 추출해서 출력
		System.out.println("파일명 : "+fstr.substring(fstr.lastIndexOf("\\")+1));
		// fstr에서 파일경로만 추출해서 출력
		System.out.println("파일경로 : "+fstr.substring(fstr.indexOf(File.separator)+1, fstr.lastIndexOf("\\")));
		...
		while(stk.hasMoreElements()) {    // 토큰요소의 값이 있다면 true, 없으면 false 리턴
