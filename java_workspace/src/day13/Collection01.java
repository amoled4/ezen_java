...
//		List list1 = new List();             // error
		...
		// index에 넣는 게 아니기 때문에 뒤에 중복되어 붙음
		for(int i=1; i<=10; i++) {
			list4.add(i);
		}
		System.out.println(list4);
		 ...
		 * 향상된 for문, Iterator처럼 순서와 상관없이 값을 가져올 수 있는 구문에서 출력가능
		 */
		System.out.println("> Iterator 출력");
		Iterator<Integer> it = list4.iterator();
		while(it.hasNext()) {   // for문 사용 불가능, hasNext() 다음 요소가 있는지 체크 true/false
		...
		// indexOf(값) : 해당 값의 list index번지를 반환 / 없다면 -1 리턴
		Integer b = 5;
